...
**Starting building (capital Palace).** A faction's **first city is its capital and is founded with a [Palace](#palace) already built** (see §Buildings). The Palace is the national wonder that marks the capital — it cannot be built manually elsewhere, so seeding it on the first city is what establishes the capital's bonuses (reduced maintenance, +1 Happiness, +4 Espionage, +8 Commerce) from turn one. The capital itself is defined as the player's earliest-founded surviving settlement (`TurnEngine._find_capital`), which is exactly that first city; if the capital is lost the Palace is not auto-rebuilt. Mechanically this is applied in `SimFacade._cmd_found_settlement`: the first settlement a player founds gets `"palace"` appended to its `structures`, provided the data tables define a `palace` entry.
